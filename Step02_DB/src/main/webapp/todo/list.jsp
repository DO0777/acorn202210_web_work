...
<style>
	...
	.icon-link svg{
		/* tranform 을 적용할때 0.4 초 동안 변화되도록 적용해라.*/
...
<body>
	<%--
		...
		-포함된 jsp 페이지에 파라미터를 전달할수 가 있다.
		-파라미터를 적절히 전달해서 포함된 jsp 페이지 내에서 해당 파라미터를 추출해서 어떤 로직을 수행할수 있다.
		-아래 같은 경우는 thisPage 라는 파라미터 명으로 todo 라는 문자열을 전달하는 것이다.
		-그러면 마치 navbar.jsp?thisPage=todo  요청한것과 비슷한 효과가 되어서
		 navbar.jsp 페이지 내부에서
		 
		 String thisPage=request.getParameter("thisPage");
		 
		 코드를 수행하면 thisPage 안에 "todo" 라는 문자열이 들어있데 된다.
	 --%>
